...
router.get("/", authenticateHybrid, async (req: AuthRequest, res) => {
  try {
    ...
    // For any connection where the agent is null, try to load from ve_voice_agents
    for (const conn of allConnections) {
      if (!conn.agent) {
        try {
          const veAgentResult = await db.execute(sql`
            SELECT id, name, language, tts_voice FROM ve_voice_agents WHERE id = ${conn.agentId} LIMIT 1
          `);
          if (veAgentResult.rows.length > 0) {
            ...
            conn.agent = {
              id: veAgent.id,
              name: veAgent.name,
              elevenLabsVoiceId: veAgent.tts_voice || null,
              language: veAgent.language || 'en',
          ...
          console.error("Error fetching ve_voice_agent for connection:", e);
        }
      }
    }

    // Fetch Plivo numbers assigned to custom voice agents
    let plivoCveConnections: any[] = [];
    try {
      const plivoCveResult = await db.execute(sql`
        SELECT 
          ppn.id as "phoneNumberId",
          ppn.phone_number as "phoneNumber",
          ppn.friendly_name as "friendlyName",
          ppn.assigned_agent_id as "agentId"
        FROM plivo_phone_numbers ppn
        WHERE ppn.user_id = ${userId} AND ppn.assigned_agent_id IS NOT NULL
      `);
      for (const row of plivoCveResult.rows) {
        // Verify if agent is custom-voice-engine
        let agentObj: any = null;
        const veAgentResult = await db.execute(sql`
          SELECT id, name, language, tts_voice FROM ve_voice_agents WHERE id = ${row.agentId} LIMIT 1
        `);
        if (veAgentResult.rows.length > 0) {
          const veAgent = veAgentResult.rows[0];
          agentObj = {
          ...
          // Check agents table
          const agentResult = await db.select().from(agents).where(eq(agents.id, row.agentId)).limit(1);
          if (agentResult.length > 0 && agentResult[0].telephonyProvider === 'custom-voice-engine') {
            agentObj = {
              id: agentResult[0].id,
              name: agentResult[0].name,
              elevenLabsVoiceId: null,
              language: agentResult[0].language || 'en',
              transferPhoneNumber: agentResult[0].transferPhoneNumber,
              transferEnabled: agentResult[0].transferEnabled,
              telephonyProvider: 'custom-voice-engine',
            };
          }
        }
        ...
        if (agentObj) {
          plivoCveConnections.push({
            id: `plivo-cve-${row.phoneNumberId}`,
            agentId: row.agentId,
      ...
      console.error("Error fetching Plivo Custom Voice connections:", e);
    }
    ...
    const connections = [
      ...allConnections.filter(c => {
        ...
        return !provider || provider === 'twilio' || provider === 'custom-voice-engine';
      }),
      ...plivoCveConnections
    ];
    ...
    // Fetch Custom Voice Engine agents
    let veAgents: any[] = [];
    try {
      const veAgentsResult = await db.execute(sql`
        SELECT id, name, language, tts_voice FROM ve_voice_agents WHERE user_id = ${userId} AND is_active = true ORDER BY created_at DESC
      `);
      ...
      console.error("Error fetching ve_voice_agents for incoming selection:", e);
    }

    // Filter out OpenAI-based agents and combine with Custom Voice agents
    const incomingAgents = [
      ...allIncomingAgents.filter(a => {
...
router.post("/", authenticateHybrid, async (req: AuthRequest, res) => {
  try {
    ...
    // Check if the phoneNumberId belongs to a Plivo phone number
    const plivoPhoneResult = await db.execute(sql`
      SELECT * FROM plivo_phone_numbers WHERE id = ${phoneNumberId} AND user_id = ${userId} LIMIT 1
    `);
    if (plivoPhoneResult.rows.length > 0) {
      const plivoPhone = plivoPhoneResult.rows[0];
      
      // Verify agent exists (in ve_voice_agents or agents)
      let agentObj: any = null;
      const veAgentResult = await db.execute(sql`
        SELECT * FROM ve_voice_agents WHERE id = ${agentId} AND user_id = ${userId} LIMIT 1
      `);
      if (veAgentResult.rows.length > 0) {
        agentObj = veAgentResult.rows[0];
        
        // Auto-insert a placeholder agent record in main 'agents' table to satisfy foreign keys
        await db.execute(sql`
          INSERT INTO agents (id, user_id, name, type, telephony_provider, language, eleven_labs_voice_id, is_active)
          VALUES (${agentObj.id}, ${userId}, ${agentObj.name}, 'incoming', 'custom-voice-engine', ${agentObj.language || 'en'}, ${agentObj.tts_voice || null}, true)
        ...
        const agentResult = await db.select().from(agents).where(and(eq(agents.id, agentId), eq(agents.userId, userId))).limit(1);
        if (agentResult.length > 0) {
          agentObj = agentResult[0];
        }
      }
      ...
      // Assign agent to Plivo phone number
      await db.execute(sql`
      ...
      return res.status(201).json({
        ...
        phoneNumber: {
          id: phoneNumberId,
          phoneNumber: plivoPhone.phone_number,
          friendlyName: plivoPhone.friendly_name,
        }
      });
    }
    ...
    if (!agent.length) {
      ...
      if (veAgentResult.rows.length > 0) {
        ...
        // Insert placeholder into agents table to satisfy foreign key constraint
        try {
          await db.execute(sql`
            INSERT INTO agents (id, user_id, name, type, telephony_provider, language, eleven_labs_voice_id, is_active, system_prompt, first_message, llm_model, temperature)
            VALUES (${veAgent.id}, ${userId}, ${veAgent.name}, 'incoming', 'custom-voice-engine', ${veAgent.language || 'en'}, ${veAgent.tts_voice || null}, ${veAgent.is_active ?? true}, ${veAgent.system_prompt || ''}, ${veAgent.first_message || ''}, ${veAgent.llm_model || 'gpt-4o-mini'}, ${veAgent.temperature || 0.7})
            ON CONFLICT (id) DO UPDATE SET
              name = EXCLUDED.name,
              language = EXCLUDED.language,
              eleven_labs_voice_id = EXCLUDED.eleven_labs_voice_id,
          ...
          // Re-query the agents table to populate the `agent` array
          agent = await db
            .select()
            .from(agents)
          ...
          console.error("Error inserting placeholder agent:", insertError);
          return res.status(500).json({ message: "Failed to initialize custom voice agent in database" });
        }
      } else {
    ...
    const isCustomVoiceEngine = agent[0].telephonyProvider === 'custom-voice-engine';
...
router.delete("/:id", authenticateHybrid, async (req: AuthRequest, res) => {
  try {
    ...
    if (id.startsWith("plivo-cve-")) {
      const plivoPhoneId = id.replace("plivo-cve-", "");
      await db.execute(sql`
        UPDATE plivo_phone_numbers SET assigned_agent_id = NULL, updated_at = NOW() WHERE id = ${plivoPhoneId} AND user_id = ${userId}
      `);
