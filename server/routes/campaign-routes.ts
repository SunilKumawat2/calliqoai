      ...
      if (sipPhoneNumberId) {
        if (agent.telephonyProvider === 'custom-voice-engine') {
          ...
          if (!userSipPhoneNumber) {
            return res.status(404).json({ error: "Custom SIP phone number not found" });
          }
    ...
    try {
      ...
      if (sipPhoneNumberId) {
        ...
        if (targetAgentId) {
          ...
          if (agent?.telephonyProvider && agent?.telephonyProvider !== 'custom-voice-engine' && sipPhone.engine !== agent.telephonyProvider) {
            return res.status(400).json({ 
              error: "Engine mismatch",
