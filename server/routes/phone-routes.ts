...
export function createPhoneRoutes(ctx: RouteContext): Router {
  ...
  const checkDemoAccount = async (req: AuthRequest, res: Response, next: any) => {
    const userId = req.userId!;
    try {
      const user = await storage.getUser(userId);
      if (user?.email === 'demo@diploy.in') {
        return res.status(403).json({
          error: "Demo Mode Active",
          message: "Modifying phone numbers is disabled in demo mode."
        });
      }
    } catch (e) {}
    next();
  ...
  router.post("/api/phone-numbers/buy", authenticateToken, checkDemoAccount, checkActiveMembership(storage), async (req: AuthRequest, res: Response) => {
    try {
  ...
  router.delete("/api/phone-numbers/:id", authenticateHybrid, checkDemoAccount, async (req: AuthRequest, res: Response) => {
    try {
