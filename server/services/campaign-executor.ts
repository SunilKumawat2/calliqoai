...
export class CampaignExecutor {
  ...
  async executeCampaign(campaignId: string): Promise<{ batchJob: BatchJob }> {
    try {
      ...
      if (agent.telephonyProvider === 'custom-voice-engine') {
        ...
        cveBatchService.executeCampaign(campaignId).then(async (result) => {
  ...
  async getBatchJobStatus(campaignId: string): Promise<BatchJobWithRecipients | null> {
    ...
    let agent = (await db.select().from(agents).where(eq(agents.id, campaign.agentId)).limit(1))[0];
    ...
    // Note: Campaign completion/failure state transition and webhooks are now handled
    // by CampaignScheduler after all individual call records are synced.
