...
// Import the 4 Figma feature card images
import salesOutreachImg from "../../assets/images/sales-outreach.jpg";
import customerSupportImg from "../../assets/images/customer-support.jpg";
import leadQualificationImg from "../../assets/images/lead-qualification.jpg";
...
function FeatureImageCard({ image, category, title, index }: FeatureImageCardProps) {
  return (
    <motion.div
      className="relative group overflow-hidden rounded-2xl border border-gray-800 hover:border-emerald-500/50 transition-all duration-300 shadow-lg shadow-black/40 aspect-[295/400] w-full cursor-pointer"
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 z-10 flex flex-col justify-end">
        <p className="text-xs font-medium text-gray-400 tracking-wider mb-1">
          {category}
        </p>
      ...
      {/* Hover border glow */}
      <div className="absolute inset-0 border border-transparent group-hover:border-emerald-500 rounded-2xl pointer-events-none transition-colors duration-300" />
    </motion.div>
  );
}
...
const CapabilityBadge = ({ text, delay }: { text: string; delay: number }) => (
  <motion.div
    ...
    transition={{ delay }}
  >
    <div className="w-4 h-4 rounded-full bg-[#1CD152]/20 flex items-center justify-center">
      <Check className="h-2.5 w-2.5 text-brand" />
    </div>
    <span className="text-brand font-medium text-sm text-[#1CD152]">{text}</span>
  </motion.div>
);
  ...
  return (
    <div className="relative">
      ...
      <div className="relative bg-gradient-to-br from-gray-900 to-gray-950 rounded-2xl border border-gray-800 p-6 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-[#1CD152]/20 rounded-lg border border-brand/20 text-[#1CD152]">
            <span className="text-sm text-brand text-[#1CD152]">{t('landing.featureSection.mockup.voice')}</span>
            <Check className="w-3 h-3 text-brand text-[#1CD152]" />
          </div>
        </div>
          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 h-1.5 bg-gray-700 rounded-full">
                <div className="w-3/4 h-full bg-[#1CD152] rounded-full" />
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500">{t('landing.featureSection.mockup.speed')}</span>
              <div className="flex-1 h-1.5 bg-gray-700 rounded-full">
                <div className="w-1/2 h-full bg-[#1CD152] rounded-full" />
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500">{t('landing.featureSection.mockup.diarization')}</span>
              <div className="flex-1 h-1.5 bg-gray-700 rounded-full">
                <div className="w-2/3 h-full bg-[#1CD152] rounded-full" />
              </div>
            </div>
          </div>
        </div>
        <Button size="sm" className="bg-[#1CD152] text-brand-foreground text-xs">
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-gray-800">
          ...
          <div className="w-8 h-8 rounded-lg bg-[#1CD152]/20 flex items-center justify-center">
        ...
        <div className="absolute inset-0 opacity-20">
          <div className="grid grid-cols-20 gap-1 p-4">
            {[...Array(200)].map((_, i) => (
              <div
                key={i}
                className={`w-1 h-1 rounded-full ${Math.random() > 0.7 ? 'bg-[#1CD152]' : 'bg-transparent'}`}
              />
            ))}
          </div>
        </div>
    ...
    <div className="relative">
      ...
      <div className="relative space-y-4">
        ...
        <div className="space-y-3 px-8">
          <div className="flex items-center gap-3 px-4 py-3 bg-[#1CD152]/20 rounded-xl border border-brand/20">
          ...
          <div className={isLeft ? "order-2 md:order-2" : "order-2 md:order-1"}>
            <div className="bg-gray-900/50 rounded-2xl border border-gray-800 p-8">
              ...
              <ul className="space-y-3 mb-8">
                {bullets.map((bullet, index) => (
                  <li key={index} className="flex items-start gap-3">
                    <div className="w-5 h-5 rounded-full bg-[#1CD152]/20 flex items-center justify-center mt-0.5 shrink-0">
                      <Check className="h-3 w-3 text-brand text-[#1CD152]" />
                    </div>
              ...
              <div className="space-y-4">
                <Link href="/login">
                  <Button
                    className="bg-transparent hover:bg-[#1CD152]/10 text-brand text-[#1CD152]
                    font-semibold border-2 border-brand/30 hover:border-brand rounded-lg px-6"
                ...
                <div className="flex items-center gap-4 text-sm text-gray-500">
                  <div className="flex items-center gap-2">
                    <Check className="h-4 w-4 text-brand text-[#1CD152]" />
...
export function FeatureSection() {
  ...
  const renderFormattedTitle = () => {
    const fullText = t('landing.featureSection.title');
    let line1 = fullText;
    let line2 = "";

    if (fullText.includes("that")) {
      const idx = fullText.indexOf("that");
      line1 = fullText.substring(0, idx + 4);
      line2 = fullText.substring(idx + 4);
    } else if (fullText.includes("जो")) {
      const idx = fullText.indexOf("जो");
      line1 = fullText.substring(0, idx);
      line2 = fullText.substring(idx);
    }

    const highlight247 = (text: string) => {
      if (text.includes("24/7")) {
        const parts = text.split("24/7");
        return (
          <>
            {parts[0]}
            <span className="text-[#1CD152] drop-shadow-[0_0_15px_rgba(16,185,129,0.3)]">24/7</span>
            {parts[1]}
          </>
        );
      }
  ...
  return (
    <section
      ref={ref}
      className="relative bg-[#02040a] pt-12 pb-16"
    ...
    >
      ...
      pt-10 pb-4 text-center">
        ...
        {/* 4 Feature Cards Row (Figma Design) - Positioned below the hero section */}
        <div className="max-w-9xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 mb-24 relative z-20">
        ...
        <div className="flex flex-wrap justify-center gap-3 text-[#1CD152] md:gap-5">
          <CapabilityBadge
            text={t('landing.featureSection.capabilities.bookMeetings')}
            delay={0}
          />
          <CapabilityBadge
            text={t('landing.featureSection.capabilities.conductInterviews')}
            delay={0.1}
          />
          <CapabilityBadge
            text={t('landing.featureSection.capabilities.coldCallProspects')}
            delay={0.2}
          />
          <CapabilityBadge
            text={t('landing.featureSection.capabilities.offerSupport')}
            delay={0.3}
          />
        </div>
      </div>
